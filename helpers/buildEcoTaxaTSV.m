...
meta_headers = {{'img_file_name', '[t]'},... % 'img_rank', '[f]'},...
  ...
  {'process_id', '[t]'}, {'process_soft', '[t]'},...
  {'process_date', '[t]'}, {'process_time', '[t]'},...
  {'process_pixel_to_micron', '[t]'}...
...
process_pixel_to_micron = global_metadata.meta.scale_bar_pixel_per_micron;
...
parfor (i_bin=1:size(feature_ids,1), parfor_arg)
  ...
  switch sample_type
    case 'inline'
      if strcmp(global_metadata.process.selection_name, 'All')
        sample_id = [global_metadata.meta.cruise_id '_INLINE'];
      else
        sample_id = [global_metadata.meta.cruise_id '_INLINE_' global_metadata.process.selection_name];
      end;
    case 'niskin'
      sample_id = [sample_profile_id '_NISKIN_' sample_niskin];
    case 'incubation'
      sample_id = [sample_profile_id '_EXPERIMENT_' sample_experiment_bottle];
    otherwise
  ...
  for i=1:size(ftr,1)
    ...
    fprintf(f, '%s\t', img_file_name, object_id, object_link, object_lat, object_lon,...
              ...
              process_id, process_soft,...
              process_date, process_time, process_pixel_to_micron,...
